...
Run from the repo root:
    ...
    # or with overrides:
    python -m ml.train_jepa --epochs 40 --d-model 128 --n-layers 3

Outputs (under ``ml/results/`` and ``ml/checkpoints/``):
...
# Repo root on sys.path so ``from ml.*`` resolves when run as ``python -m ml.train_jepa``.

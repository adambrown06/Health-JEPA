...
# Repo root on sys.path when run as ``python -m ml.qdrant_eval``
...
class QdrantEvalConfig:
    ...
    qdrant_path: str = "ml/qdrant_local"
...
def _load_checkpoint(cfg: QdrantEvalConfig, device: torch.device):
    ...
    if not ckpt_path.exists():
        raise FileNotFoundError(
            f"Checkpoint not found: {ckpt_path}. Run `python -m ml.train_jepa` first."
        )

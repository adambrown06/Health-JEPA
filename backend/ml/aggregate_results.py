...
plus one run with sequence baselines for the non-JEPA comparators, and writes:
  ...
  - backend/ml/results/paper_tables/ablation_training.md + .tex + .csv
  - backend/ml/results/paper_tables/outcome_main.md + .tex + .csv
  - backend/ml/results/paper_tables/outcome_per_outcome.md + .tex
  - backend/ml/results/paper_tables/summary.json
...
RESULTS = Path("backend/ml/results")
...
def build_retrieval_table() -> tuple[str, str]:
    ...
    if not paper.exists():
        return "", ""
    d = json.loads(paper.read_text(encoding="utf-8"))

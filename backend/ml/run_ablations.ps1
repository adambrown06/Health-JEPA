...
foreach ($r in $runs) {
    ...
    python -m backend.ml.train_jepa `
        ...
        --sigreg-lambda $r.sig `
        --orth-lambda $r.orth 2>&1 | Tee-Object -FilePath "backend/ml/results/ablation_$($r.tag).log"
}

Write-Host ""
Write-Host "[done] Ablation matrix complete."
Get-ChildItem backend/ml/checkpoints/jepa_best_*.pt | Select-Object Name, Length

...
python -m backend.ml.outcome_eval `
    --checkpoint "backend/ml/checkpoints/jepa_best_full.pt" `
    --tag "full" `
    --seq-epochs 30 2>&1 | Tee-Object -FilePath "backend/ml/results/outcome_run_full.log"
...
foreach ($tag in $tags) {
    ...
    python -m backend.ml.outcome_eval `
        --checkpoint "backend/ml/checkpoints/jepa_best_$tag.pt" `
        --tag $tag `
        --skip-sequence-baselines 2>&1 | Tee-Object -FilePath "backend/ml/results/outcome_run_$tag.log"
}
